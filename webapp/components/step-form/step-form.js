...
const StepForm = () => {
	...
	const uploadPatientList = (e) => {
		...
		console.log("uploadPatientList files:", file);
		setPatientListCollection(Object.assign({}, patientListCollection, {[file[0].name]: file}));
		setSelectedList(file[0].name);
	}
	
	const uploadSurvey = (e) => {
		const file = Array.from(e.target.files);
		console.log("uploadSurvey files:", file);
		setPatientListCollection(Object.assign({}, surveyCollection, {[file[0].name]: file}));
		setSelectedSurvey(file[0].name);
	}
	...
	return <div className={styles.container}>
		...
		>
			...
			{(patientListNames.length > 1) && <p className={styles.listtitle}>Choose patient list</p>}
			
			{patientListNames.map(name =>
				<div key={uuidV4()} className={styles.filename}>
					<input
						type="checkbox"
						value={name}
						onChange={() => selectCurrentList(name)}
						checked={name===selectedList}
						disabled={patientListNames.length === 1}
					/>
					{name}
				</div>
			)}
		...
		>
			...
			{(surveyNames.length > 1) && <p className={styles.listtitle}>Choose survey</p>}
			...
			{surveyNames.map(name =>
				<div key={uuidV4()} className={styles.filename}>
					<input
						type="checkbox"
						value={name}
						onChange={() => selectCurrentSurvey(name)}
						checked={name===selectedList}
						disabled={surveyNames.length === 1}
					/>
					{name}
				</div>
			)}
